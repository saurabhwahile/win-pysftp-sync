...
class Connection(object):
    ...
    def get_r(self, remotedir, localdir, preserve_mtime=False):
        ...
        for fname in wtcb.flist:
            ...
            #MODIFIED
            #Support for windows filesystem at the server and client
            localFilePath = reparent(localdir, fname.replace(remotedir, ''))
            localDirPath = localFilePath[:localFilePath.rfind('\\')]

            fname = fname.replace('\\', '/')
            #Make folder path locally if not present
            if not os.path.exists(localDirPath):
                os.makedirs(localDirPath)

            try:
                if os.stat(localFilePath).st_size == self.stat(fname).st_size:
                    continue
            except OSError:
                pass
            ...
            try:
                ...
                raise paramiko.ssh_exception.SSHException('IOError')
            except:
                raise paramiko.ssh_exception.SSHException('Unknown Error')
